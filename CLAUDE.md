...
給 n8n 排程呼叫的端點（如抓取銀行利率），**排程與時間由 n8n 決定**，後端程式只負責「執行、回傳資料、回報狀態」：

- 回傳內容：`message`（抓取訊息，只寫「做什麼」如「抓取五大公股銀行利率」，不寫成敗）、`status`（文字「成功」或「失敗」，不用 true／false）、`success_count`／`fail_count`（成功／失敗筆數），失敗時另有 `error`（原因），再加上資料本身（如 `rates` 清單，格式方便 n8n 直接顯示）。共用格式定義於 `backend/app/services/n8n_result.py`。
- **失敗時的欄位與成功時在同一層**（不包 `detail`，含金鑰錯誤 401／503），n8n 才能用同一組運算式處理成功與失敗；失敗用 `fail_error` 建立，由 `main.py` 的例外處理轉成回應。
- **不回傳**執行／抓取時間；需要時間時由 n8n 端自己取得。
- n8n 只負責「啟動」與「顯示結果狀態」，呼叫時**不帶任何參數**（除了 `X-API-Key`）；起訖日期等一切由後端自己決定。
- 不在後端做排程（不用 cron、APScheduler 等），也不自行決定「何時執行」。
- 失敗時以 HTTP 狀態碼（如 502、500）加中文錯誤訊息回報，讓 n8n 能判斷成敗。
