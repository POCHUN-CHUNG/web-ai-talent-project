...
from app.services.profile_ai import fail_if_stale, mark_pending, reset_to_pending, run_description_job
from app.services.questionnaire import QUESTIONNAIRE_VERSION, QUESTIONS, compute_profile, validate_answers
...
def enforce_submit_limit(user_id: int, key: str = "questionnaire") -> None:
    # 【送出限流】每位使用者每分鐘只能送出一次問卷；超過回 429。
    # 參數：user_id=使用者編號、key=限流種類（questionnaire＝送出問卷、description＝重新產生描述，各自獨立計時）
...
def serialize_profile(p: RiskProfile) -> dict:
    # 【風險屬性轉回應】把資料庫的一列轉成契約的 RiskProfile 格式。參數：p=風險屬性快照
    return {
        "id": p.id,
        "questionnaireAnswerId": p.questionnaire_answer_id,
        ...
        "descriptionStatus": p.description_status,
...
def submit_answers(
    ...
    # 【送出問卷】驗證作答 → 轉換規則 → 作答與風險屬性同一交易寫入 → 背景請 AI 產生描述。
    # 參數：body={"answers": {...}}、user=目前登入者、db=資料庫連線
    # 1. 驗證 14 題（缺題、選項錯誤、Q11 規則不符皆回 400，且不寫入任何資料）
    try:
    ...
    # 3. 只有作答完整無衝突（ready）才計入限流並呼叫 AI；作答衝突（limited）要讓使用者立刻修正重送，
    #    且他不會進結果頁、用不到描述，所以不限流也不呼叫 AI（D-76）。填錯被 400 擋回的同樣不計
    needs_ai = result["readiness"] == "ready"
    if needs_ai:
        enforce_submit_limit(user.id)
    # 4. 作答與風險屬性在同一交易寫入（每次重填都新增一組，舊的保留）
    ...
    profile = RiskProfile(
        ...
        description_status="pending" if needs_ai else "failed",  # limited 不產生描述
    )
    db.add(profile)
    db.flush()
    if needs_ai:
    ...
    db.commit()
    # 5. 回應送出後才呼叫 AI，不讓使用者等待；失敗時描述標為 failed
    if needs_ai:
        background.add_task(run_description_job, profile.id)
    return {"riskProfileId": profile.id, "readiness": profile.readiness}


@router.post("/risk-profiles/{profile_id}/regenerate-description", status_code=202,
...
def regenerate_description(
    ...
    # 【重新產生解析】AI 描述失敗時，讓使用者再要一次；描述已成功或仍在產生中則不動作。
    ...
    if p.description_status != "failed":
        return {"descriptionStatus": p.description_status}
    # 3. 限流（每人每分鐘 1 次）→ 改回 pending → 背景重新呼叫 AI
    enforce_submit_limit(user.id, "description")
    if reset_to_pending(p.id):
        mark_pending(p.id)
        background.add_task(run_description_job, p.id)
    return {"descriptionStatus": "pending"}
