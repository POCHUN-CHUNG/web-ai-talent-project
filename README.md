...
| 抓取每日股價與大盤指數 | 依股票基本資料建立清單（上市、上櫃向 yfinance 抓，指數向證交所抓；基本資料是空的就什麼都不抓），抓取個股還原收盤價與大盤報酬指數，起訖日由後端自行決定（首次上線從 2003-01-01 抓到今天，之後個股與大盤都只抓近 1 個月；個股若因除權息使還原價基準改變，後端會自動整檔重抓（不需 n8n 處理，也不回報）），同一代號同一天直接覆寫，所有股價資料永久保留，不執行刪除 |
...
**中斷與被擋的處理**：每個來源失敗會自動重試；每一批抓完立即寫入資料庫；失敗的部分維持舊資料不動、下次執行會整段補回；連續多批失敗會判定被封鎖並中止；同一時間只允許執行一次。有任何失敗時回傳失敗狀態，讓 n8n 依流程重試或通知。日常執行約數分鐘；首次上線要從 2003-01-01 抓到今天，需數十分鐘，n8n 呼叫節點的逾時（Timeout）請設 60 分鐘以上。

## 登入、註冊與設定

首頁前是登入／註冊頁（同一頁面切換），採玻璃卡（Glass Console）視覺風格，支援淺色／深色模式。登入後可在「設定」頁查看帳號、修改密碼，以及手動切換深色模式（未手動切換時預設跟隨系統設定，選擇會記住在瀏覽器）。
...
- 前端顏色、圓角、字級、間距等設計 token 全部集中在 `frontend/src/styles/tokens.css`（依據 `DESIGN.md`），其餘檔案一律以 `var(--token-name)` 引用，不寫死色碼；新增畫面請比照既有的 `components/ui/*` 共用元件。
