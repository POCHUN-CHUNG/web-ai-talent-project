...
| `POST /risk-profiles/{id}/regenerate-description` | 每使用者每分鐘 1 次（D-73，與送出問卷獨立計時） | `rate:description:{user_id}` |
...
- [ ] B12a 送出問卷後顯示「分析中」遮罩，描述完成前不顯示指標；完成後才一次顯示四項指標與描述，且無交叉分析區塊
...
- [ ] B17 同一使用者 1 分鐘內連續送出兩次有效問卷，第二次回 429 `RATE_LIMITED` 且不建立任何列；被 400 擋回的送出、以及結果為 `limited` 的送出（不呼叫 AI）不計入限流，`limited` 後立即修正重送須成功（須自動化）
- [ ] B18 剛送出問卷後於 1 分鐘內按「重新填寫問卷」，不進入問卷頁，跳出「填寫的時間過短」視窗並倒數，關閉後仍可查看結果；冷卻結束後可進入；冷卻查詢端點未登入回 401
- [ ] B19 已有可用風險屬性者進問卷頁有「取消」按鈕，按下回結果頁；第一次填寫與 `limited` 使用者不顯示該按鈕
- [ ] B20 描述失敗時「風險屬性解析」區塊顯示「重新產生」按鈕、指標照常顯示；按下後指標維持顯示、不出現「分析中」遮罩，解析區塊顯示「產生中…」，成功即更新為解析文字、再失敗則回到按鈕；1 分鐘內再按回 429 並顯示訊息；他人的風險屬性回 403；狀態非 failed 時不重複呼叫 AI
- [ ] B21 描述停在 `pending` 且「產生中」標記已過期（如背景工作因後端重啟中斷）時，讀取風險屬性會改判為 `failed`，使用者可按「重新產生」；標記未過期或狀態非 `pending` 時不改（須自動化）
