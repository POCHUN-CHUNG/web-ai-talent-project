...
| OpenAI 回傳內容 | Structured Outputs 嚴格 schema ＋ 後端內容檢查（段落、引用白名單、finding 與核心指標覆蓋）＋ 照抄欄位比對（分析報告） |
...
| 程式碼 | 不得寫死任何金鑰或預設密碼 |
| 日誌 | 密碼、通行證、`N8N_API_KEY`、`OPENAI_API_KEY` **一律不得寫入日誌**，包含例外堆疊 |
...
| 成本失控 | 重試上限 2 次、問卷送出每使用者每分鐘 1 次、報告快取 24 小時、`ready` 後不重打。**`max_output_tokens` 設為模型上限 65,536，不構成有效上限**，成本控制完全由前述四項承擔 |
...
| 作答衝突 | Q10／Q11 兩種衝突（含只勾「其他商品」、有年數卻從未投資）、Q3／Q9 衝突、多項同時衝突 → 422 `ANSWER_CONFLICT`，不存檔 |
| 承受意願－下跌反應 | 3 級意願 × 6 種反應共 18 格，窮舉 |
| Prompt 對照 | 每個選項原文、等級文字、fact／finding id 都逐字出現在 Prompt 字典；`rules_version` 一致 |
...
- [ ] B12a 送出問卷後結果頁只顯示標題與轉圈圈（無文字、無遮罩），外框以背景漸層色閃爍；解析完成前不顯示指標與按鈕；完成後才一次顯示四項指標、按鈕與四段有標題的解析，外框閃爍停止，且無交叉分析區塊
- [ ] B12b 從首頁進入結果頁顯示最新一份，按鈕為「回首頁」「重新填寫問卷」；重填送出後為「回首頁」「新增投資組合」
- [ ] B12 AI 解析產生成功時 `sectionsStatus` 為 `ready`，恰好四段且順序固定，每段 80–360 字（Prompt 目標 180–300 字），五個 finding 與四個核心指標都有被引用
- [ ] B13 AI 輸出連續不合規時，共呼叫 3 次（間隔 2、5 秒）後 `sectionsStatus` 為 `failed`，四項核心指標**仍正常顯示**；金鑰錯誤等不值得重試的失敗只呼叫 1 次（須自動化）
- [ ] B14 AI 輸出的 `fact_ids`、`finding_ids` 全部存在於當次輸入（須自動化）
- [ ] B17 同一使用者 1 分鐘內連續送出兩次有效問卷，第二次回 429 `RATE_LIMITED` 且不建立任何列；被 400、422 擋回的送出不存檔、不呼叫 AI、不計入限流，修正後立即重送須成功（須自動化）
- [ ] B18 剛送出問卷後於 1 分鐘內按「重新填寫問卷」，不進入問卷頁，跳出「填寫的時間過短」視窗並倒數，關閉後仍可查看結果；冷卻結束後可進入；冷卻查詢端點未登入回 401
- [ ] B19 問卷頁底部左側有「取消」、右側有「送出」；按「取消」回風險屬性頁
- [ ] B19a 作答衝突時相關題目紅框標示（紅字寫出與第幾題矛盾），並跳出只有「確認」的提示視窗（說明寫出題號），按下後自動捲到第一個衝突題；改了一組衝突中的任一題，整組紅框一起消失並自動捲到與它矛盾的那一題；還有衝突紅框時按「送出」不送出、再次跳出提示視窗；未作答時該題紅框並捲到第一題
- [ ] B19b 註冊成功直接進入風險屬性頁且不跳「尚未完成風險屬性評估」提示；登入成功進入 `/`（沒有風險屬性時才被導回並跳提示）
- [ ] B20 解析失敗時解析區塊顯示錯誤說明與「重新產生」按鈕（成功時不顯示按鈕）、指標照常顯示；按下後指標維持顯示、外框閃爍直到結果回來，解析區塊顯示「產生中…」，成功即更新為四段解析、再失敗則回到按鈕；1 分鐘內再按回 429 並顯示訊息；他人的風險屬性回 403；狀態非 failed 時不重複呼叫 AI
- [ ] B21 解析停在 `pending` 且「產生中」標記已過期（如背景工作因後端重啟中斷）時，讀取風險屬性會改判為 `failed`，使用者可按「重新產生」；標記未過期或狀態非 `pending` 時不改（須自動化）
- [ ] B15 重填問卷後產生新的 `risk_profiles` 列，舊列仍存在且內容未變
- [ ] B16 Q10=A 但 Q11 勾選「股票」或「其他商品」時，送出回 422 `ANSWER_CONFLICT` 並帶 `questionIds: ["Q10","Q11"]`，兩張表都不新增任何列；問卷頁標示這兩題
...
- [ ] E22 單次呼叫的輸出 token 數不超過 `OPENAI_MAX_OUTPUT_TOKENS`
...
| Q7 | 作答衝突：兩種衝突情境（含只勾「其他商品」）各至少一例，加上不衝突情境 |
