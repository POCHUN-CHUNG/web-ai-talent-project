...
| 資料 | 來源 | 取得方式 | 備註 |
| --- | --- | --- | --- |
| 個股與 ETF 日收盤價 | yfinance | 由柏鈞自行撰寫抓取程式 | 一律取 `Adj Close`（已還原除權息）。非官方 API，可能中斷（R-03） |
| 市場基準 IR0001 | 臺灣證券交易所《發行量加權股價報酬指數》 | `https://www.twse.com.tw/indicesReport/MFI94U?response=json&date=YYYYMM01` | ✅ 已驗證回傳結構。一次一個月；日期為民國年；數值含千分位逗號 |
| 股票基本資料 | 臺灣證券交易所 ISIN 分類表 | `https://isin.twse.com.tw/isin/class_main.jsp?market={m}&issuetype={i}` | ✅ 柏鈞提供抓取程式。四組參數涵蓋上市普通股（含 KY）、上櫃普通股、上市 ETF、上櫃 ETF |
